...
public interface StatsRepository extends JpaRepository<EndpointHit, Integer> {
    @Query("select new main.java.ru.practicum.model.ViewStats(hit.app, hit.uri, count(distinct hit.ip)) " +
    ...
    @Query("select new main.java.ru.practicum.model.ViewStats(hit.app, hit.uri, count(hit.ip)) " +
            "from EndpointHit as hit " +
            "where hit.timestamp between ?1 AND ?2 " +
            "group by hit.app, hit.uri " +
